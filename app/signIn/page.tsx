...
export default function Home() {
  ...
  const handleSignUp = async (formValues:Object) => {
    ...
    <div className="flex text-white min-h-screen flex-col items-center justify-between p-24 px-0 bg-black ">
      <main className=' rounded-md shadow-white  w-11/12 max-w-lg'>
        <h1 className='flex text-2xl font-bold border-2 '>
          <span className='bg-white w-2/4 text-black text-right pr-2'>Registro</span>
              ...
              <Form className='flex flex-col items-center gap-6'>
                ...
                <div className='grid grid-cols-1 gap-6 mt-6 '>
                    <div className='flex flex-col'>
                        <Field name="firstName" type="text" required className="outline-none bg-transparent border-b-2 p-0" placeholder="Nombre" />
                        
                        <ErrorMessage name='firstName' render={(message) => <div className="text-red-600 flex items-center">{message} <CgDanger/> </div>}/>
                    </div>

                    <div className='flex flex-col'>
                        <Field name="lastName" type="text" required className="outline-none bg-transparent border-b-2 p-0" placeholder="Apellido"/>

                        <ErrorMessage name='lastName' render={(message) => <div className="text-red-600 flex items-center">{message} <CgDanger/> </div>}/>
                    </div>
                    ...
                    <div className='flex flex-col'>
                        <Field name="confirmPassword" type="password" required className="outline-none bg-transparent border-b-2 p-0" placeholder="Confirmar contraseña"/>

                        <ErrorMessage name='confirmPassword' render={(message) => <div className="text-red-600 flex items-center">{message} <CgDanger/> </div>}/>
                    </div>

                </div>

                <button type="submit" className='col-span-2 border-2 p-2'>Crear Usuario</button>
              </Form>
            )}
        </Formik>
      </main>
